...
def sidebar_navigation():
    lottie_book = load_lottie_url("http://assets9.lottiefiles.com/temp/1f20_aAafin.json")
    if lottie_book:
        with st.sidebar:
    ...
    if search_term:
        with st.spinner("Searching..."):
            ...
            if st.session_state.search_results:
                ...
                st.markdown("<div class='warning-message'>No books found matching the search.</div>", unsafe_allow_html=True)
...
# Stats view
    ...
    if stats['total_books'] == 0:
        st.markdown("<div class='warning-message'>No books available to generate stats.</div>", unsafe_allow_html=True)
    else:
        create_visualization(stats)

# Add Book view
elif st.session_state.current_view == "add":
    st.markdown("<h2 class='sub-header'>Add a Book</h2>", unsafe_allow_html=True)
    book_title = st.text_input("Title")
    book_authors = st.text_input("Authors")
    book_year = st.number_input("Publication Year", min_value=1000, max_value=9999)
    book_genre = st.text_input("Genre")
    book_read_status = st.radio("Read status", ("Read", "Unread"))

    if st.button("Add Book"):
        if book_title and book_authors and book_year and book_genre:
            add_book(book_title, book_authors, book_year, book_genre, book_read_status == "Read")
            st.success(f"Book '{book_title}' added successfully!")
        else:
            st.warning("All fields are required to add a book.")
